...
import numpy as np
import heapq
# from heapp import MeldableHeap
import collections
from os import path
from os import sys
import math
...
class NetworkSimulatorEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        if next_node == current_event.dest:
            ...
            # print "were pushing this onto the queue(step): source:{}, node:{}, dest:{}".format(current_event.source, current_event.node, current_event.dest)
            # print "with priority:{}".format((current_time+1.0, -self.events))
            current_event.qtime = current_time
            # self.event_queue.add((current_time, current_event))
    ...
    def _reset(self):
        ...
        if self.callmean == 1.0:
            inject_event.etime = -math.log(random.random()) #change to 1.0 for hardcoding
        else:
            inject_event.etime = -math.log(1- random.random())*float(self.callmean) #change to 1.0 for hardcoding


        self.events = 1
        # print "were pushing this onto the queue(reset): source:{}, node:{}, dest:{}".format(inject_event.source, inject_event.node, inject_event.dest)
        # print "with priority:{}".format((1.0, -self.events))

        inject_event.qtime = 0.0
        heapq.heappush(self.event_queue,((1.0, -self.events), inject_event))
        self.injections += 1
        self.events += 1
        # self.event_queue.add((0.0, inject_event))
    ...
    def start_packet(self, time):
        ...
        # source = self.sources[self.next_source]
        # dest = self.dests[self.next_dest]
        # self.next_dest += 1
        # self.next_source += 1
        #
    ...
    def get_new_packet_bump(self):
        ...
        while current_event.source == INJECT :
             if self.callmean == 1.0 or self.callmean == 0.0:
                 current_event.etime += -math.log(1 - random.random()) #change to 1.0 for hardcoding
             else:
                 current_event.etime += -math.log(1- random.random())*float(self.callmean) #change to 1.0 for hardcoding
